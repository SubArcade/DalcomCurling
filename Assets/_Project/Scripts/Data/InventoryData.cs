...
public class DonutEntry
{
    ...
    [Tooltip("도넛 레벨")][FirestoreProperty] public int level { get; set; }
    // 앤트리에서만 적용되는 값
    // 기존 SO에서 값 받고 수정한 값
...
{
    [field: SerializeField, Tooltip("도넛 앤트리")][FirestoreProperty] public List<DonutEntry> donutEntries { get; set; } = new();  // <- [도넛1] [2] [3] [4] [5]
    //캐릭터
    //파티클
}
